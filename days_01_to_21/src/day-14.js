...
function indexOfKey(n, salt, hashMethod) {
  const keyIndices = [];
  let i = 0, potentialKeys = [];
  while (keyIndices.length < n - 1 || i < keyIndices[n] + 1000) {
    potentialKeys = potentialKeys.filter(k => k !== null)
      .filter(({ index }) => index + 1000 >= i);
    const hash = hashMethod(salt + i);
    potentialKeys.forEach(({ triple, index }, j) => {
      if (hash.includes(triple.repeat(5))) {
        keyIndices.push(index);
        keyIndices.sort((x, y) => x - y);
        potentialKeys[j] = null;
        console.log(`Found ${keyIndices.length} keys.`);
      }
    });
    const maybeTriple = getTripleOrNull(hash)
    if (maybeTriple) { potentialKeys.push({ index: i, triple: maybeTriple }); }
    i++;
  }
  return(keyIndices[n]);
}
...
export function part2(input) {
  return indexOfKey(63, input, s => {
    let hash = s;
    for (let i = 0; i < 2017; i++) { hash = MD5.hash(hash); }
    return hash;
  });
}
