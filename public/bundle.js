...
function allowedIPs(blacklist) {
  return blacklist.reduce(function (_ref, _ref2) {
    var top = _ref.top,
    ...
    for (var i = top; i < lower; i++) {
      allowed.push(i);
    }
    return { top: Math.max(upper + 1, top), allowed: allowed };
  }, { top: 0, allowed: [] }).allowed;
}

function part1$19(input) {
  return allowedIPs(input)[0];
}

function part2$19(input) {
  return allowedIPs(input).length;
}
...
time = Date.now();
console.log("--- part 1 ---");
console.log("solution: " + part1$19(input20) + ", time: " + (Date.now() - time) + "ms\n");
console.log("--- part 2 ---");
time = Date.now();
console.log("solution: " + part2$19(input20) + ", time: " + (Date.now() - time) + "ms\n");
