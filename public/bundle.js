...
(counting from 0) should be swapped. swap letter X with letter Y means that the
letters X and Y should be swapped (regardless of where they appear in the
string). rotate left/right X steps means that the whole string should be
rotated; for example, one right rotation would turn abcd into dabc. rotate based
on position of letter X means that the whole string should be rotated to the
...
additional time if the index was at least 4. reverse positions X through Y means
that the span of letters at indexes X through Y (including the letters at X and
Y) should be reversed in order. move position X to position Y means that the
letter which is at index X should be removed from the string, then inserted such
that it ends up at index Y. For example, suppose you start with abcde and
perform the following operations:

swap position 4 with position 0 swaps the first and last letters, producing the
input for the next step, ebcda. swap letter d with letter b swaps the positions
of d and b: edcba. reverse positions 0 through 4 causes the entire string to be
reversed, producing abcde. rotate left 1 step shifts all letters left one
position, causing the first letter to wrap to the end of the string: bcdea. move
position 1 to position 4 removes the letter at position 1 (c), then inserts it
at position 4 (the end of the string): bdeac. move position 3 to position 0
removes the letter at position 3 (a), then inserts it at position 0 (the front
of the string): abdec. rotate based on position of letter b finds the index of
letter b (1), then rotates the string right once plus a number of times equal to
that index (2): ecabd. rotate based on position of letter d finds the index of
letter d (4), then rotates the string right once, plus a number of times equal
to that index, plus an additional time because the index was at least 4, for a
total of 6 right rotations: decab. After these steps, the resulting scrambled
password is decab.
...
function swap(w, x, y) {
  var _sort = [x, y].sort(function (x, y) {
    return x - y;
  }),
...
function unrotateBasedOn(w, x) {
  var i = [1, 3, 5, 7, 2, 4, 6, 0].indexOf(x);
  return i >= 4 ? rotate(w, 1, i + 2) : rotate(w, 1, i + 1);
}

function reverse(w, x, y) {
  var middle = w.slice(x, y + 1).split("").reverse().join("");
  return w.slice(0, x) + middle + w.slice(y + 1);
}

function move$3(w, x, y) {
  var letters = w.split(""),
      _letters$splice = letters.splice(x, 1),
      _letters$splice2 = slicedToArray(_letters$splice, 1),
      letter = _letters$splice2[0];
  letters.splice(y, 0, letter);
  return letters.join("");
}

function scramble(word, _ref) {
  var op = _ref.op,
      params = _ref.params;

  var indices = params.map(function (x) {
    return typeof x === "number" ? x : word.indexOf(x);
  });
  if (op === "swap") {
    return swap.apply(undefined, [word].concat(toConsumableArray(indices)));
  }
  if (op === "rotate left") {
    return rotate.apply(undefined, [word, 1].concat(toConsumableArray(indices)));
  }
  if (op === "rotate right") {
    return rotate.apply(undefined, [word, -1].concat(toConsumableArray(indices)));
  }
  if (op === "rotate based on") {
    return rotateBasedOn.apply(undefined, [word].concat(toConsumableArray(indices)));
  }
  if (op === "reverse") {
    return reverse.apply(undefined, [word].concat(toConsumableArray(indices)));
  }
  if (op === "move") {
    return move$3.apply(undefined, [word].concat(toConsumableArray(indices)));
  }
}

function unscramble(word, _ref2) {
  var op = _ref2.op,
      params = _ref2.params;
  ...
  if (op === "rotate based on") {
    return unrotateBasedOn.apply(undefined, [word].concat(toConsumableArray(indices)));
  }
  if (op === "reverse") {
    return reverse.apply(undefined, [word].concat(toConsumableArray(indices)));
  }
  if (op === "move") {
    return move$3.apply(undefined, [word].concat(toConsumableArray(indices.reverse())));
  }
}

function part1$20(input) {
  return input.reduce(scramble, "abcdefgh");
}

function part2$20(input) {
  return input.reverse().reduce(unscramble, "fbgdceah");
}
...
// import * as day22 from "./day-22.js";
// import input22 from "./input-22.json";
// import * as day23 from "./day-23.js";
// import input23 from "./input-23.json";
// import * as day24 from "./day-24.js";
// import input24 from "./input-24.json";
// import * as day25 from "./day-25.js";
// import input25 from "./input-25.json";
...
// time = Date.now();
// console.log("--- part 1 ---");
// console.log(`solution: ${day19.part1(input19)}, time: ${Date.now() - time}ms\n`);
// console.log("--- part 2 ---");
// time = Date.now();
// console.log(`solution: ${day19.part2(input19)}, time: ${Date.now() - time}ms\n`);
// console.log("--- DAY 20 ---");
// time = Date.now();
// console.log("--- part 1 ---");
// console.log(`solution: ${day20.part1(input20)}, time: ${Date.now() - time}ms\n`);
// console.log("--- part 2 ---");
// time = Date.now();
// console.log(`solution: ${day20.part2(input20)}, time: ${Date.now() - time}ms\n`);
console.log("--- DAY 21 ---");
time = Date.now();
console.log("--- part 1 ---");
console.log("solution: " + part1$20(input21) + ", time: " + (Date.now() - time) + "ms\n");
console.log("--- part 2 ---");
time = Date.now();
console.log("solution: " + part2$20(input21) + ", time: " + (Date.now() - time) + "ms\n");
// console.log("--- DAY 22 ---");
// time = Date.now();
// console.log("--- part 1 ---");
// console.log(`solution: ${day22.part1(input22)}, time: ${Date.now() - time}ms\n`);
// console.log("--- part 2 ---");
// time = Date.now();
// console.log(`solution: ${day22.part2(input22)}, time: ${Date.now() - time}ms\n`);
// console.log("--- DAY 23 ---");
// time = Date.now();
// console.log("--- part 1 ---");
// console.log(`solution: ${day23.part1(input23)}, time: ${Date.now() - time}ms\n`);
// console.log("--- part 2 ---");
// time = Date.now();
// console.log(`solution: ${day23.part2(input23)}, time: ${Date.now() - time}ms\n`);
// console.log("--- DAY 24 ---");
// time = Date.now();
// console.log("--- part 1 ---");
// console.log(`solution: ${day24.part1(input24)}, time: ${Date.now() - time}ms\n`);
// console.log("--- part 2 ---");
// time = Date.now();
// console.log(`solution: ${day24.part2(input24)}, time: ${Date.now() - time}ms\n`);
// console.log("--- DAY 25 ---");
// time = Date.now();
// console.log("--- part 1 ---");
// console.log(`solution: ${day25.part1(input25)}, time: ${Date.now() - time}ms\n`);
// console.log("--- part 2 ---");
// time = Date.now();
// console.log(`solution: ${day25.part2(input25)}, time: ${Date.now() - time}ms\n`);
