...
function age(multipliers) {
  return multipliers.filter(m => m.ttl > 0)
    .map(m => ({ ...m, ttl: m.ttl - 1 }));
}
...
function calculateDecompressedLength(singlePass, input) {
  /* Rather than actually expanding our input, we calculate how many times
    ...
    list of multipliers. */
  let buffer = "", decompressedLength = 0, multipliers = [];
  for (let i = 0; i < input.length; i++) {
    ...
    const shouldIgnoreMarkers = singlePass && multipliers.length;
    if (input[i] === ")" && !shouldIgnoreMarkers) {
      const [ttl, k] = parseMarker(buffer);
      buffer = "";
      multipliers.push({ ttl, k });
    } else if (buffer || (!shouldIgnoreMarkers && input[i] === "(")) {
      buffer += input[i];
    } else {
      decompressedLength += multipliers.reduce((acc, m) => acc * m.k, 1);
    }
  }
