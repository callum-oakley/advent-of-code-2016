...
function hash(state) {
  ...
  return positions.sort().reduce((x, y) => x + y);
}
...
class StateTracker {
  ...
  seen(state) {
    ...
    if (this.traversed.has(hashedState)) { return true; }
...
function move(state, selection, direction) {
  const newState = {
...
function possibleSteps(state) {
  const selections = possibleSelections(state)
    .filter(s => [1, 2].includes(s.length));
  if (state.elevator === "floor 4") {
    return selections.map(s => move(state, s, -1));
  } else if (state.elevator === "floor 1") {
    return selections.map(s => move(state, s, 1));
  }
  return [
    ...selections.map(s => move(state, s, 1)),
    ...selections.map(s => move(state, s, -1))
  ];
}

function breadthFirstSearch(initialState) {
  let states = [initialState], steps = 0;
...
export const part1 = breadthFirstSearch;
...
export function part2(input) {
  return breadthFirstSearch({
    ...
    "floor 1": [
      ...input["floor 1"],
      { "element": "elerium", "device": "microchip" },
      { "element": "elerium", "device": "generator" },
      { "element": "dilithium", "device": "microchip" },
      { "element": "dilithium", "device": "generator" }
    ]
  });
}
